...
impl InMemoryGenerator {
    ...
    fn gen_size_check(
        ...
        if requires_rebuild {
            ...
            // Nothing extra on a successful in-place write: an in-memory table
            // has no CDC stream to publish it to. The persisted generator passes
            // the operation there instead. See `opaque_rebuild`.
