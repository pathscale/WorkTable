...
impl Generator {
    ...
    pub fn get_persistence_engine_type(&self) -> TokenStream {
        ...
        let space_index_type = if self.attributes.pk_arctic_string
            || (self.attributes.pk_unsized && self.attributes.pk_wti_logical)
        {
            quote! {
                SpaceLogicalIndexUnsized<#primary_key_type, { #inner_const_name as u32 }, { #page_const_name as u32 }>
            }
        } else if self.attributes.pk_unsized {
            quote! {
                SpaceIndexUnsized<#primary_key_type, { #inner_const_name as u32 }, { #page_const_name as u32 }>
            }
        } else if self.attributes.pk_wti_logical || self.attributes.pk_arctic {
            quote! {
                SpaceLogicalIndex<#primary_key_type, { #inner_const_name as u32 }, { #page_const_name as u32 }>
            }
            ...
            quote! {
                SpaceIndex<#primary_key_type, { #inner_const_name as u32 }, { #page_const_name as u32 }>
            }
        };
