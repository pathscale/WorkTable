...
//!
//! This is ported from `worktable-vec`'s `hydrate` module, which is where the
//! format was designed and where its tests still live. It is reproduced rather
...
//! fit in that page**, then an 8 byte directory at the tail. Nothing spans a
//! boundary.
//!
...
//! # What is checked
//!
//! Every page carries a CRC-32 of its body, and every header field is
...
//! key list. These pages carry a row-type fingerprint where a space file
//! carries a space id, so a WorkTable reader sees an id it does not recognise,
//! which is the honest outcome: they are not its rows.
...
///
/// Sixteen entries rather than 256: this runs once per 16 KiB page, so the
/// table is cache noise and the loop is not the cost of anything.
...
/// That is a real duplication, and the risk is a layout drifting apart in two
/// places, which is why the bytes above are written down and
/// `the_header_matches_databuckets_layout` checks them.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct Header {
...
fn rows_per_page<R>(rows: &[R], hint: usize) -> usize
where
    Vec<R>: Codec,
    R: Clone,
{
...
fn page_rows<R>(raw: &[u8], index: usize, schema: &mut Option<u32>) -> Result<Vec<R>, LoadError>
where
    Vec<R>: Codec,
{
