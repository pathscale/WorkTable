...
// ─────────────────────────────────────────────────────────────────────
// Version 1: only id + name (persisted so we can write test data)
// ─────────────────────────────────────────────────────────────────────
mod v1 {
    use super::*;
...
// ─────────────────────────────────────────────────────────────────────
// Version 2: id + name + email (persisted so we can write test data)
// ─────────────────────────────────────────────────────────────────────
mod v2 {
    use super::*;
...
// ─────────────────────────────────────────────────────────────────────
// Current (v3): id + name + email + created_at
// ─────────────────────────────────────────────────────────────────────
worktable!(
    name: User,
    persist: true,
...
// ─────────────────────────────────────────────────────────────────────
// Migration impls (neighbor pairs only)
// ─────────────────────────────────────────────────────────────────────
pub struct UserMigration;

impl Migration<v1::UserV1Row, v2::UserV2Row> for UserMigration {
    type Context = UserMigrationContext;

    fn migrate(row: v1::UserV1Row, ctx: &Self::Context) -> v2::UserV2Row {
        v2::UserV2Row {
            id: row.id,
            name: row.name,
...
impl Migration<v2::UserV2Row, UserRow> for UserMigration {
    type Context = UserMigrationContext;

    fn migrate(row: v2::UserV2Row, ctx: &Self::Context) -> UserRow {
        UserRow {
            id: row.id.into(),
            name: row.name,
            email: row.email,
...
migration_engine!(
    ...
    version_tables: {
        1 => v1::UserV1WorkTable,
        2 => v2::UserV2WorkTable,
    },
);
...
fn test_migrate_v1_to_current() {
    ...
    runtime.block_on(async {
        ...
        let dst = "tests/data/migration/v1_to_current_new";
        ...
        // Write v1 data
        {
            ...
            // Ensure all background writes are flushed
        ...
        // Verify migrated data
        {
            ...
            for row in &rows {
                assert_eq!(row.email, "unknown@example.com");
