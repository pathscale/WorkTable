...
macro_rules! runtime_backend_suite {
    ($module:ident, $label:literal $(, runtime: $backend:tt $(($flavor:tt))?)?) => {
        mod $module {
            ...
            // `update_in_place` has somewhere to write that no index watches.
            worktable!(
                name: RuntimeMatrix,
                persist: false,
            ...
            async fn every_mutation_runs() {
                ...
                table
                    .update_by_id(ids[0], RuntimeMatrixColumns::BUCKET, 3)
                    .await
                    .unwrap();
                ...
                for _ in 0..64 {
                    table
                        .update_in_place_by_id(ids[1], RuntimeMatrixColumns::COUNTER, |counter| *counter += 1u64)
                        .await
                        .unwrap();
                }
            ...
            async fn a_persisted_table_survives_a_reload() {
...

                {
                    ...
                    table
                        .update_by_id(1, RuntimeMatrixPersistColumns::BUCKET, 3)
                        .await
                        .unwrap();
                    table
                        .update_in_place_by_id(2, RuntimeMatrixPersistColumns::COUNTER, |counter| *counter = 4_242u64.into())
                        .await
                        .unwrap();
