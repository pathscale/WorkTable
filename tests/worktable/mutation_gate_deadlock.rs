...
//! `update`/`update_in_place`/`delete` paths hold the resulting `MutationGuard` inside
    ...
    runtime.block_on(async move {
        ...
        let ta = {
            ...
            tokio::spawn(async move {
                for i in 0..500u64 {
                    table.update_by_id(a, GateBenchColumns::VAL, i).await.unwrap();
                }
            })
        };
        let tb = {
            ...
            tokio::spawn(async move {
                for i in 0..500u64 {
                    table.update_by_id(b, GateBenchColumns::VAL, i).await.unwrap();
                }
            })
        };
    ...
    runtime.block_on(async move {
        ...
        for worker in 0..8u64 {
            ...
            handles.push(tokio::spawn(async move {
                for i in 0..300u64 {
                    table.update_by_id(key, GateBenchColumns::VAL, i).await.unwrap();
                }
            }));
        }
