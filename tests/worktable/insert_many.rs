...
#[tokio::test]
async fn insert_many_returns_pks_and_reads_see_every_row() {
...
#[tokio::test]
async fn insert_many_of_no_rows_is_ok() {
    let table = BatchWorkTable::default();
...
async fn collision_case(collide_at: usize) {
    let table = BatchWorkTable::default();
    ...
    table
        .insert_many((0..5).map(|i| row(i, 200 + i)).collect())
        .await
        .unwrap();
...
#[tokio::test]
async fn unique_collision_at_first_row_rejects_the_batch() {
    collision_case(0).await;
}

#[tokio::test]
async fn unique_collision_at_middle_row_rejects_the_batch() {
    collision_case(2).await;
}

#[tokio::test]
async fn unique_collision_at_last_row_rejects_the_batch() {
    collision_case(4).await;
}

#[tokio::test]
async fn unique_collision_between_two_batch_rows_rejects_the_batch() {
    let table = BatchWorkTable::default();
...
#[tokio::test]
async fn duplicate_primary_key_inside_the_batch_rejects_the_batch() {
    let table = BatchWorkTable::default();
...
#[tokio::test]
async fn duplicate_primary_key_with_an_existing_row_rejects_the_batch() {
    let table = BatchWorkTable::default();
...
#[tokio::test]
async fn reserve_pks_hands_out_contiguous_keys_that_interleave_with_get_next_pk() {
    let table = BatchWorkTable::default();
...
#[tokio::test]
async fn concurrent_reader_never_observes_a_rejected_batch() {
    const BATCH: u64 = 16;
...
#[tokio::test]
async fn concurrent_reader_sees_successful_batches_in_prefix_order() {
    const BATCH: u64 = 32;
