...
use data_bucket::{INNER_PAGE_SIZE, Link, UnsizedIndexPage, parse_page};
...
async fn test_index_page_read_after_create_node_in_space_index() {
    ...
    let page =
        parse_page::<UnsizedIndexPage<String, DEFAULT_PAGE_STRIDE>, { INNER_PAGE_SIZE as u32 }, DEFAULT_PAGE_STRIDE>(
            &mut file, 2,
        )
        .await
        .unwrap();
...
async fn test_index_page_read_after_insert_at_removed_place_in_space_index() {
    let mut file =
        worktable::prelude::fsx::open("tests/data/expected/space_index_unsized/process_insert_at_removed_place.wt.idx")
            .await
            .unwrap();
...
async fn test_index_pages_read_after_node_split() {
    let mut file = worktable::prelude::fsx::open("tests/data/expected/space_index_unsized/process_split_node.wt.idx")
        .await
        .unwrap();
