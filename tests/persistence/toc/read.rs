...
async fn test_index_table_of_contents_read() {
    let mut file = worktable::prelude::fsx::open("tests/data/expected/persist_index_table_of_contents.wt.idx")
        .await
        .unwrap();
    let next_id_gen = Arc::new(AtomicU32::new(1));
    let toc = IndexTableOfContents::<u32, { INNER_PAGE_SIZE as u32 }, DEFAULT_PAGE_STRIDE>::parse_from_file(
        &mut file,
        0.into(),
        next_id_gen,
    )
...
async fn test_index_table_of_contents_read_from_space_index() {
    ...
    let toc = IndexTableOfContents::<(u32, Link), { INNER_PAGE_SIZE as u32 }, DEFAULT_PAGE_STRIDE>::parse_from_file(
        &mut file,
        0.into(),
        next_id_gen,
    )
...
async fn test_index_table_of_contents_read_from_space_index_with_remove_node() {
    let mut file = worktable::prelude::fsx::open("tests/data/expected/space_index/process_remove_node.wt.idx")
        .await
        .unwrap();
    let next_id_gen = Arc::new(AtomicU32::new(2));
    let toc = IndexTableOfContents::<(u32, Link), { INNER_PAGE_SIZE as u32 }, DEFAULT_PAGE_STRIDE>::parse_from_file(
