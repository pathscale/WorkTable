...
fn paged_mutation_shapes_fail_before_emission() {
    for query in [
        "update_partial: { Change(value) by value }",
        "update_partial_in_place: { Change(value) by value }",
        "update_partial_in_place: { Change(id) by id }",
    ] {
...
fn a_vec_query_cannot_silently_ignore_a_runtime_profile() {
    let checked = check(
        "name: T, vec: true, columns: { id: u64 primary_key, value: u64 }, queries: { update_partial runtime scheduled: { Change(value) by id } }",
    );
...
fn supported_paged_index_updates_remain_valid() {
    let checked = check(
        "name: T, columns: { id: u64 primary_key, value: u64, amount: u64 }, indexes: { value_idx: value }, queries: { update_partial: { Change(amount) by value } }",
    );
