        ...
        if let TokenTree::Group(ops) = ops {
            ...
            while let Some(ident) = parser.peek_next() {
                match ident.to_string().as_str() {
                    "update_partial" => {
                        let (runtime, updates) = parser.parse_update_partials()?;
                        queries.update_partials = updates;
                        queries.update_partial_runtime = runtime;
                    }
                    "delete" => {
                    ...
                    "update_partial_in_place" => {
                        let (runtime, updates) = parser.parse_update_partials_in_place()?;
                        queries.update_partials_in_place = updates;
                        queries.update_partial_in_place_runtime = runtime;
                    }
                    other => {
                        return Err(syn::Error::new(
                            ident.span(),
                            format!(
                                "Unexpected token `{other}`; expected one of `update_partial`, `delete`, `update_partial_in_place`"
                            ),
                        ));
                    }
                }
...
mod tests {
    ...
    fn sections_are_unannotated_by_default() {
        ...
        assert!(queries.update_partial_runtime.is_none());
        assert!(queries.delete_runtime.is_none());
        assert!(queries.update_partial_in_place_runtime.is_none());
    }
    ...
    fn each_section_takes_a_runtime_annotation() {
        let tokens = quote! {
            queries: {
                ...
                update_partial_in_place runtime bulk: { Bump(qty) by id },
            }
        };
        let queries = Parser::new(tokens).parse_queries().unwrap();

        assert_eq!(queries.update_partial_runtime.unwrap(), "fast_local");
        assert_eq!(queries.delete_runtime.unwrap(), "wide");
        assert_eq!(queries.update_partial_in_place_runtime.unwrap(), "bulk");
        assert_eq!(queries.update_partials.len(), 1);
        assert_eq!(queries.deletes.len(), 1);
        assert_eq!(queries.update_partials_in_place.len(), 1);
    }
    ...
    fn a_section_rejects_a_backend_in_place_of_a_profile() {
        let tokens = quote! {
            queries: {
                update_partial runtime nagoya: { Fill(qty) by id },
            }
        };
    ...
    #[test]
    fn legacy_update_section_is_rejected() {
        let tokens = quote! {
            queries: { update: { Fill(qty) by id } }
        };
        let error = Parser::new(tokens).parse_queries().unwrap_err().to_string();

        assert!(error.contains("Unexpected token `update`"), "{error}");
    }
