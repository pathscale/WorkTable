...
fn art_primary_key_ranges(c: &mut Criterion) {
    ...
    for id in 0..ROWS {
        futures::executor::block_on(congee.insert(CongeeRangeBenchmarkRow { id, value: id })).unwrap();
        futures::executor::block_on(arctic.insert(ArcticRangeBenchmarkRow { id, value: id })).unwrap();
    }
